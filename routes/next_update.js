...
module.exports = [
    {
        name: "/next_update",
        export(req, res) {
            let duration = moment.duration(config.endpoints.current_map_schedule.regular.end_time-Date.now()/1000, "minutes").format("HH:mm");
            let object = { hours: "00", minutes: duration.split(":")[0] || "00", seconds: duration.split(":")[1] || "00", next_update_ts: config.endpoints.current_map_schedule.regular.end_time*1000, latest_update_ts: config.endpoints.current_map_schedule.regular.start_time*1000 };
